...
class Holder:
    ...
    def exec_instruction(self):
        ...
        bots_exist = False
        bots_exist = check_bot_exist(instruction[0])
        bots_exist = check_bot_exist(instruction[1])
        if bots_exist:
            if len(self.items) == 2:
                chip_low = ''
                chip_high = ''
...
# Check if any bot still have microchips
def endloop_check(loop):
    if loop == 0:
        return False
    for bot in holders:
        if bot.name.startswith('bot'):
            if bot.count_items() > 0:
                return False
    return True
